...
ใช้ LittleFS Uploader เช่นเดียวกับโปรเจกต์ CryptoClock หลัก:

1. เปิด `data/hello-display-mode.txt`
2. ใส่ค่า `setup` เพื่อให้จอถามสีใหม่, `black` เพื่อบังคับพื้นหลังดำ หรือ `white` เพื่อบังคับพื้นหลังขาว
3. เปิด `HelloWiFi_Starter.ino` แล้วกด `⌘⇧P` → **Upload LittleFS to Pico/ESP8266/ESP32**

คำสั่งนี้อัปโหลดเฉพาะไฟล์ตั้งค่าใน `data/` จึงไม่ต้อง Upload โค้ด `.ino` ใหม่
