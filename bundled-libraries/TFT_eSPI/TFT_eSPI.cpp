...
if (c == ',') {
  ...
  // 🔹 ปรับการแสดงผลของ ,
  if (c == ',') {
    for (int8_t i = 0; i < 3; i++) { // ลดจาก 5 เป็น 3
        uint8_t line = pgm_read_byte(font + (c * 5) + i + 1); // ข้าม 0x00 ตัวแรก
    ...
    x += 3 * size; // 🔹 ลดช่องว่างของตัวอักษรถัดไป
    return; 
  }
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  if (utf8 == '\n') {
    ...
    int advance = drawChar(uniCode, cursor_x, cursor_y, textfont);
    if (utf8 == ',') advance -= 10; // 🔹 ลดช่องว่างของ ,
    cursor_x += advance;
  }
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    if(utf8 == '\n') {
      ...
      int16_t advance = pgm_read_byte(&glyph->xAdvance) * (int16_t)textsize;
      if (uniCode == ',') advance -= 7; // 🔹 ลดช่องว่างของ ,
      cursor_x += advance;
          }
  }
#endif // LOAD_GFXFF
